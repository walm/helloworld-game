...
- (id)init//WithWidth:(float)width height:(float)height
{
    if ((self = [super init]))
    {
...
- (void)showMenu
{
    ...
    [[Sparrow.juggler delayInvocationAtTarget:mTitle byTime:1.0f] fadeIn:nil];
    
    [Sparrow.juggler addObject:mBgStars];
    [mBgStars start];
}
...
- (void)startGame
{
    ...
    // clear stage
    [Sparrow.juggler removeObjectsWithTarget:mPlayStage];
    ...
    [[Sparrow.juggler delayInvocationAtTarget:self byTime:15.0f] speedUpGame];
}
...
- (void)addUFOWithContinued:(BOOL)continued
{
    ...
    if (continued)
        [[Sparrow.juggler delayInvocationAtTarget:self byTime:mUfoDelay] addUFOWithContinued:YES];
}
...
- (void)speedUpGame
{
    ...
    [[Sparrow.juggler delayInvocationAtTarget:self byTime:5.0f] speedUpGame];
}
...
- (void)onUFOExplode:(SPEvent*)event
{
    ...
    [Sparrow.juggler removeObjectsWithTarget:ufo];
